...
BEGIN
  ...
  -- Call center counts: time-based
  -- dropped_count = ALL orders in period (every order counts as dropped)
  -- confirmed_count = orders with confirmation_status = 'confirmed' in period
  -- A confirmed order is counted in BOTH dropped and confirmed
  SELECT
    COUNT(*)::integer,
  ...
  -- dropped_count = ALL orders (no subtraction)
  ...
  -- Note: v_dropped_count is overwritten above for invoice-linked stats display only.
  -- For call center fees we use v_all_period_count (all orders) and v_period_confirmed_count.
  ...
  -- Call center fees: dropped = ALL orders, confirmed = confirmed orders
  -- Confirmed orders are counted TWICE (once in dropped, once in confirmed)
