...
BEGIN
  ...
  -- CRITICAL: Period-bounded to prevent double-counting across invoices
  ...
  -- Cross-invoice: count only delivered for revenue — PERIOD BOUNDED
  ...
  -- UNIFIED shipping breakdown: combine direct + cross-invoice shipped orders
  WITH all_shipped AS (
    -- Direct orders
    ...
    UNION ALL
    -- Cross-invoice orders — PERIOD BOUNDED
    SELECT COALESCE(p.weight_kg, CASE WHEN p.weight='up_to_1kg' THEN 0.5 WHEN p.weight='up_to_2kg' THEN 1.5 WHEN p.weight='up_to_3kg' THEN 2.5 WHEN p.weight='above_3kg' THEN 3.5 ELSE 0.5 END)*o.quantity AS total_wt
    FROM public.orders o JOIN public.invoices inv_orig ON inv_orig.id = o.invoice_id
    LEFT JOIN public.products p ON p.seller_id = o.seller_id AND p.name = o.product_name
    WHERE o.seller_id = v_invoice.seller_id AND o.invoice_id != p_invoice_id
  ...
  -- Total shipping fees from the unified breakdown
  SELECT COALESCE(SUM((item->>'fee')::numeric), 0) INTO v_shipping_fees
  FROM jsonb_array_elements(v_shipping_breakdown) AS item;

  -- Update shipped count to include cross
  v_shipped_count := v_shipped_count + v_cross_shipped_count;

  -- Cross delivered revenue (ONLY delivered) — PERIOD BOUNDED
  ...
  -- Adjustments: now include shipping_difference in the output AND in the net calculation
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
