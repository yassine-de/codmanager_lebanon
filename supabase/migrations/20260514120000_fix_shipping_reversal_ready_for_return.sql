-- Fix: ready_for_return / return / failed_attempt / out_for_delivery should NOT
-- trigger a shipping_reversal adjustment. The package was physically shipped in
-- all of these states, so the shipping fee should remain charged.
...
BEGIN
  ...
  IF OLD.delivery_status IS DISTINCT FROM NEW.delivery_status THEN
    ...
    -- Shipping reversal: was shipped → status that means "never shipped at all"
    -- NOTE: ready_for_return, return, out_for_delivery, failed_attempt all mean
    -- the package WAS shipped — do NOT reverse the shipping fee for these.
    IF v_was_shipped
