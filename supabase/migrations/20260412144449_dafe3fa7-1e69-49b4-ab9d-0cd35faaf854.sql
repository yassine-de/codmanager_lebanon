...
BEGIN
  ...
  -- All counts use COUNT(DISTINCT) for safety
  SELECT COUNT(DISTINCT o.id) INTO v_total_orders_count FROM public.orders o WHERE o.invoice_id = p_invoice_id;
  SELECT COUNT(DISTINCT o.id) INTO v_delivered_count FROM public.orders o WHERE o.invoice_id = p_invoice_id AND o.delivery_status = 'delivered';
  ...
  -- Cross-invoice shipped (COUNT(DISTINCT o.id) via subquery)
