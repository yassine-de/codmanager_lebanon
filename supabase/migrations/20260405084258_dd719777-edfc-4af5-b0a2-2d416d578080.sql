...
DECLARE
  ...
  v_period_dropped_count integer := 0;
...
BEGIN
  ...
  -- Call center counts: time-based, ALL orders for seller in invoice period
  SELECT
    COUNT(*)::integer,
  ...
  v_period_dropped_count := v_all_period_count - v_period_confirmed_count;

  -- Invoice-linked order stats (for delivery/shipping/revenue)
  ...
  -- Shipping breakdown with weight fallback
  WITH invoice_orders AS (
    SELECT o.quantity,
      COALESCE(p.weight_kg,
        CASE
  ...
  -- Use period-based counts for call center fees
  v_call_center_fees := (COALESCE(v_period_confirmed_count, 0) * COALESCE(v_confirmed_rate, 0)) + (COALESCE(v_period_dropped_count, 0) * COALESCE(v_dropped_rate, 0));
  ...
  RETURN jsonb_build_object(
    ...
    'counts', jsonb_build_object('total_orders_count', COALESCE(v_all_period_count, 0), 'delivered_count', COALESCE(v_delivered_count, 0), 'shipped_count', COALESCE(v_shipped_count, 0), 'confirmed_count', COALESCE(v_period_confirmed_count, 0), 'dropped_count', COALESCE(v_period_dropped_count, 0)),
    'call_center_breakdown', jsonb_build_object('confirmed_count', COALESCE(v_period_confirmed_count, 0), 'confirmed_rate', COALESCE(v_confirmed_rate, 0), 'confirmed_fees', COALESCE(v_period_confirmed_count, 0) * COALESCE(v_confirmed_rate, 0), 'dropped_count', COALESCE(v_period_dropped_count, 0), 'dropped_rate', COALESCE(v_dropped_rate, 0), 'dropped_fees', COALESCE(v_period_dropped_count, 0) * COALESCE(v_dropped_rate, 0)),
