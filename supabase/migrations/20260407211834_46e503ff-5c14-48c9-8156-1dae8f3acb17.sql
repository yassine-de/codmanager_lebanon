...
BEGIN
  ...
  -- Determine shipped status transitions
  ...
  -- Helper: get seller rates
  SELECT * INTO v_seller_rates
  FROM public.seller_rates
  WHERE user_id = OLD.seller_id
  LIMIT 1;

  -- Calculate shipping fee for a given quantity
  -- (reused in multiple cases below)

  -- Check if order was shipped within the closed invoice period
  SELECT EXISTS (
  ...
  -- Calculate shipping fees for old and new quantities when needed
  ...
  IF OLD.delivery_status IS DISTINCT FROM NEW.delivery_status THEN
    ...
    -- Delivery reversal: delivered → non-delivered
    ...
    -- Shipping reversal: was shipped → no longer shipped
    IF v_was_shipped AND NOT v_now_shipped AND v_has_shipment_in_closed_invoice THEN
    ...
    IF v_diff != 0 OR v_shipping_diff != 0 THEN
      INSERT INTO public.invoice_adjustments (
        ...
        CASE WHEN v_diff != 0 THEN 0 ELSE 0 END,
        v_diff,
        CASE WHEN v_shipping_diff != 0 THEN v_prev_shipping ELSE 0 END,
        CASE WHEN v_shipping_diff != 0 THEN 0 ELSE 0 END,
        v_shipping_diff,
        v_reason, 'pending'
  ...
  THEN
    ...
    -- Revenue delta if order is delivered
    IF NEW.delivery_status = 'delivered' THEN
      v_diff := (NEW.price * NEW.quantity) - (OLD.price * OLD.quantity);
    END IF;

    -- Shipping delta if order was shipped in this closed invoice and quantity changed bracket
