-- 1. RPC: drop 'booked' from the visible pipeline
...
-- 2. Trigger: drop 'booked' from the auto-create pipeline
...
AS $function$
DECLARE
  v_pipeline text[] := ARRAY['shipped','in_transit','out_for_delivery','with_courier','delivered','failed_attempt','returned','return','ready_for_return'];
BEGIN
...
-- 3. Cleanup: remove follow-up records and assignments for orders still at 'booked'
DELETE FROM public.order_follow_ups fu
USING public.orders o
WHERE fu.order_id = o.order_id
  AND o.delivery_status = 'booked';

UPDATE public.orders
SET follow_up_assigned_to = NULL,
    follow_up_assigned_at = NULL
WHERE delivery_status = 'booked'
