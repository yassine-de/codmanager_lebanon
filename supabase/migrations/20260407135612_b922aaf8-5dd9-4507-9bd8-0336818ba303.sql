...
BEGIN
  ...
  -- Delivered orders (for revenue - only delivered)
  ...
  -- Revenue from direct delivered ONLY
  ...
  -- Shipping breakdown: for ALL shipped+ orders (not just delivered)
  WITH shipped AS (
  ...
  -- Shipping fees total: for ALL shipped+ orders
  ...
  FROM (SELECT COALESCE(p.weight_kg, CASE WHEN p.weight='up_to_1kg' THEN 0.5 WHEN p.weight='up_to_2kg' THEN 1.5 WHEN p.weight='up_to_3kg' THEN 2.5 WHEN p.weight='above_3kg' THEN 3.5 ELSE 0.5 END)*o.quantity AS total_wt
    FROM public.orders o LEFT JOIN public.products p ON p.seller_id = o.seller_id AND p.name = o.product_name
    WHERE o.invoice_id = p_invoice_id AND o.delivery_status = ANY(v_shipped_statuses)) sub;

  -- Cross-invoice: orders from closed invoices that were shipped AFTER closure
  WITH cross_shipped AS (
    ...
    WHERE o.seller_id = v_invoice.seller_id AND o.invoice_id != p_invoice_id
      ...
      AND EXISTS (SELECT 1 FROM public.order_history oh WHERE oh.order_id = o.order_id AND oh.field_changed = 'delivery_status' AND oh.new_value = ANY(v_shipped_statuses) AND oh.created_at > inv_orig.finalized_at)
  )
  SELECT COUNT(*),
  ...
  -- Cross-invoice: count only delivered for revenue
  ...
  -- Cross shipping fees: for ALL shipped+ cross-invoice orders
  ...
  FROM (SELECT o.quantity, COALESCE(p.weight_kg, CASE WHEN p.weight='up_to_1kg' THEN 0.5 WHEN p.weight='up_to_2kg' THEN 1.5 WHEN p.weight='up_to_3kg' THEN 2.5 WHEN p.weight='above_3kg' THEN 3.5 ELSE 0.5 END) AS weight_kg
    ...
    WHERE o.seller_id = v_invoice.seller_id AND o.invoice_id != p_invoice_id AND inv_orig.status IN ('ready','paid') AND inv_orig.finalized_at IS NOT NULL
      AND o.delivery_status = ANY(v_shipped_statuses)
      AND EXISTS (SELECT 1 FROM public.order_history oh WHERE oh.order_id = o.order_id AND oh.field_changed = 'delivery_status' AND oh.new_value = ANY(v_shipped_statuses) AND oh.created_at > inv_orig.finalized_at)) sub;

  v_shipping_fees := v_shipping_fees + v_cross_shipping_fees;

  -- Cross delivered revenue (ONLY delivered)
