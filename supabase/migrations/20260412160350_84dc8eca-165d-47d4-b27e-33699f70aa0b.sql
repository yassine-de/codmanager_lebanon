...
BEGIN
  ...
  -- SHIPPED COUNT: COUNT(DISTINCT o.id)
  ...
  -- DROPPED COUNT: COUNT(DISTINCT o.id)
  ...
  -- CONFIRMED COUNT (direct): COUNT(DISTINCT o.id)
  ...
  -- CONFIRMED COUNT (cross-invoice): COUNT(DISTINCT o.id)
  ...
  -- Cross-invoice shipped (with time-bounded delivery check)
  WITH cross_shipped AS (
    SELECT o.id, o.order_id, o.customer_name, o.customer_phone, o.product_name, o.quantity, o.price, o.total_amount, o.created_at,
      o.delivery_status,
      COALESCE(p.weight_kg, CASE WHEN p.weight='up_to_1kg' THEN 0.5 WHEN p.weight='up_to_2kg' THEN 1.5 WHEN p.weight='up_to_3kg' THEN 2.5 WHEN p.weight='above_3kg' THEN 3.5 ELSE NULL END) AS weight_kg,
      inv_orig.invoice_number AS original_invoice_number,
      -- Reusable flag: was this order delivered within this invoice period?
      (o.delivery_status = 'delivered' AND EXISTS (
  ...
  SELECT COUNT(DISTINCT id),
    -- Only include in delivered_orders list if delivery event happened in this period
    ...
    -- Use is_delivered_in_period for was_delivered flag
  ...
  -- Shipping breakdown: deduplicated via subqueries with DISTINCT
