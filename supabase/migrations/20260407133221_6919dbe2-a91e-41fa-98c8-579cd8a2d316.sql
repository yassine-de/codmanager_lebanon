...
BEGIN
  ...
  -- All orders with was_delivered (uses delivered_at OR order_history)
  WITH invoice_orders AS (
  ...
  -- Cross-invoice orders
  WITH cross_shipped AS (
    SELECT o.id, o.order_id, o.customer_name, o.customer_phone, o.product_name, o.quantity, o.price, o.total_amount, o.created_at, o.delivery_status,
      COALESCE(p.weight_kg, CASE WHEN p.weight='up_to_1kg' THEN 0.5 WHEN p.weight='up_to_2kg' THEN 1.5 WHEN p.weight='up_to_3kg' THEN 2.5 WHEN p.weight='above_3kg' THEN 3.5 ELSE NULL END) AS weight_kg,
    ...
    WHERE o.seller_id = v_invoice.seller_id AND o.invoice_id != p_invoice_id AND inv_orig.status IN ('ready','paid') AND inv_orig.finalized_at IS NOT NULL
      AND o.delivery_status IN ('shipped','delivered')
      AND EXISTS (SELECT 1 FROM public.order_history oh WHERE oh.order_id = o.order_id AND oh.field_changed = 'delivery_status' AND oh.new_value IN ('shipped','delivered') AND oh.created_at > inv_orig.finalized_at)
  )
  SELECT COUNT(*) FILTER (WHERE delivery_status='shipped'), COUNT(*) FILTER (WHERE delivery_status='delivered'),
  ...
  INTO v_cross_shipped_count, v_cross_delivered_count, v_cross_delivered_orders, v_cross_orders FROM cross_shipped;
  ...
  FROM (SELECT o.quantity, COALESCE(p.weight_kg, CASE WHEN p.weight='up_to_1kg' THEN 0.5 WHEN p.weight='up_to_2kg' THEN 1.5 WHEN p.weight='up_to_3kg' THEN 2.5 WHEN p.weight='above_3kg' THEN 3.5 ELSE 0.5 END) AS weight_kg
    ...
    WHERE o.seller_id = v_invoice.seller_id AND o.invoice_id != p_invoice_id AND inv_orig.status IN ('ready','paid') AND inv_orig.finalized_at IS NOT NULL AND o.delivery_status IN ('shipped','delivered')
      AND EXISTS (SELECT 1 FROM public.order_history oh WHERE oh.order_id = o.order_id AND oh.field_changed = 'delivery_status' AND oh.new_value IN ('shipped','delivered') AND oh.created_at > inv_orig.finalized_at)) sub;
  ...
  INTO v_adjustments, v_adjustment_net FROM public.invoice_adjustments WHERE applied_invoice_id = p_invoice_id;
